...
void QuaternionApp::Update(const GameTimer& gt)
{
	OnKeyboardInput(gt);
...
void QuaternionApp::Draw(const GameTimer& gt)
{
	// 获取当前的指令分配器
...
void QuaternionApp::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void QuaternionApp::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void QuaternionApp::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void QuaternionApp::OnKeyboardInput(const GameTimer& gt)
{
...
void QuaternionApp::UpdateObjectCBs(const GameTimer& gt)
{
...
void QuaternionApp::UpdateMaterialBuffer(const GameTimer& gt)
{
...
void QuaternionApp::UpdateMainPassCB(const GameTimer& gt)
{
...
void QuaternionApp::BuildShapeGeometry()
{
...
void QuaternionApp::DrawRenderItems(ID3D12GraphicsCommandList * cmdList, const std::vector<RenderItem*> & ritems)
{
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 6> QuaternionApp::GetStaticSamplers()
{
